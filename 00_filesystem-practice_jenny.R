...
## Copy again, using fs ----
library(fs)
(from_dir <- path_home("Desktop", "day1_s1_explore-libraries")) # extra () forces a print (asigns and prints)
(from_files <- dir_ls(from_dir, glob = "*.R"))
(to_files <- path_file(from_files))
(out <- file_copy(from_files, to_files)) # get the paths themselves
dir_ls()
dir_info()

## want to have "standards" and "taste" for your file names...
## intential _ & - for file names and directories
